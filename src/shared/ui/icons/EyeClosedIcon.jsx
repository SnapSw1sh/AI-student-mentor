const PNG_DATA_URI = 'data:image/png;base64,';

export function EyeClosedIcon({ className }) {
  return (
    <svg
      className={className}
      width="21"
      height="18"
      viewBox="0 0 21 18"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      aria-hidden="true"
    >
      <rect width="21" height="18" fill="url(#pattern_eye_closed)" fillOpacity="0.5" />
      <defs>
        <pattern id="pattern_eye_closed" patternContentUnits="objectBoundingBox" width="1" height="1">
          <use xlinkHref="#image_pattern_eye_closed" transform="matrix(0.00857143 0 0 0.01 0.0714286 0)" />
        </pattern>
        <image
          id="image_pattern_eye_closed"
          width="100"
          height="100"
          preserveAspectRatio="none"
          xlinkHref={PNG_DATA_URI}
        />
      </defs>
    </svg>
  );
}
